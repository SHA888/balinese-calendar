...
impl Ekawara {
    pub fn from_jdn(_jdn: i64) -> Self { Ekawara::Luang }
    pub fn name(&self) -> &'static str { "Luang" }
    pub fn urip(&self) -> u8 { 1 }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Dwiwara { Menga = 0, Pepet = 1 }
...
impl Dwiwara {
    ...
    pub fn name(&self) -> &'static str {
        match self { Dwiwara::Menga => "Menga", Dwiwara::Pepet => "Pepet" }
    }
    pub fn urip(&self) -> u8 {
        match self { Dwiwara::Menga => 5, Dwiwara::Pepet => 4 }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Triwara { Pasah = 0, Beteng = 1, Kajeng = 2 }
...
impl Triwara {
    ...
    pub fn urip(&self) -> u8 {
        match self { Triwara::Pasah => 9, Triwara::Beteng => 4, Triwara::Kajeng => 7 }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Caturwara { Sri = 0, Laba = 1, Jaya = 2, Menala = 3 }
...
impl Caturwara {
    ...
    pub fn urip(&self) -> u8 {
        match self { Caturwara::Sri => 6, Caturwara::Laba => 5, Caturwara::Jaya => 8, Caturwara::Menala => 9 }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Pancawara { Umanis = 0, Paing = 1, Pon = 2, Wage = 3, Kliwon = 4 }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Sadwara { Tungleh=0, Aryang=1, Urukung=2, Paniron=3, Was=4, Maulu=5 }
...
impl Sadwara {
    ...
    pub fn urip(&self) -> u8 {
        match self {
            Sadwara::Tungleh => 7, Sadwara::Aryang => 6, Sadwara::Urukung => 8,
            Sadwara::Paniron => 9, Sadwara::Was    => 3, Sadwara::Maulu   => 5,
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Astawara { Sri=0, Indra=1, Guru=2, Yama=3, Ludra=4, Brahma=5, Kala=6, Uma=7 }
...
impl Astawara {
    pub fn from_jdn(jdn: i64) -> Self {
        ...
        let adjusted = if pd < 63 { pd }
                       else if pd < 126 { pd + 1 }
                       else if pd < 189 { pd + 2 }
                       else { pd + 3 };
        match adjusted % 8 {
            0 => Astawara::Sri,    1 => Astawara::Indra, 2 => Astawara::Guru,
            3 => Astawara::Yama,   4 => Astawara::Ludra, 5 => Astawara::Brahma,
            6 => Astawara::Kala,   _ => Astawara::Uma,
        }
    }
    pub fn name(&self) -> &'static str {
        match self {
            Astawara::Sri    => "Sri",   Astawara::Indra  => "Indra",
            Astawara::Guru   => "Guru",  Astawara::Yama   => "Yama",
            Astawara::Ludra  => "Ludra", Astawara::Brahma => "Brahma",
            Astawara::Kala   => "Kala",  Astawara::Uma    => "Uma",
        }
    }
    pub fn urip(&self) -> u8 {
        match self {
            Astawara::Sri    => 6, Astawara::Indra  => 5, Astawara::Guru   => 8,
            Astawara::Yama   => 9, Astawara::Ludra  => 3, Astawara::Brahma => 7,
            Astawara::Kala   => 1, Astawara::Uma    => 4,
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Sangawara {
    Dangu=0, Jangur=1, Gigis=2, Nohan=3, Ogan=4,
    Erangan=5, Urungan=6, Tulus=7, Dadi=8,
}
...
impl Sangawara {
    pub fn from_jdn(jdn: i64) -> Self {
        ...
        match adjusted {
            0 => Sangawara::Dangu,   1 => Sangawara::Jangur,  2 => Sangawara::Gigis,
            3 => Sangawara::Nohan,   4 => Sangawara::Ogan,    5 => Sangawara::Erangan,
            6 => Sangawara::Urungan, 7 => Sangawara::Tulus,   _ => Sangawara::Dadi,
        }
    }
    pub fn name(&self) -> &'static str {
        match self {
            Sangawara::Dangu    => "Dangu",   Sangawara::Jangur   => "Jangur",
            Sangawara::Gigis    => "Gigis",   Sangawara::Nohan    => "Nohan",
            Sangawara::Ogan     => "Ogan",    Sangawara::Erangan  => "Erangan",
            Sangawara::Urungan  => "Urungan", Sangawara::Tulus    => "Tulus",
            Sangawara::Dadi     => "Dadi",
        }
    }
    pub fn urip(&self) -> u8 {
        match self {
            Sangawara::Dangu    => 5,  Sangawara::Jangur  => 8, Sangawara::Gigis   => 9,
            Sangawara::Nohan    => 3,  Sangawara::Ogan    => 7, Sangawara::Erangan => 1,
            Sangawara::Urungan  => 4,  Sangawara::Tulus   => 6, Sangawara::Dadi    => 8,
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Dasawara {
    Pandita=0, Pati=1, Suka=2, Duka=3, Sri=4,
    Manuh=5, Manusa=6, Raja=7, Dewa=8, Raksasa=9,
}
...
impl Dasawara {
    ...
    pub fn from_wewaran(pancawara: &Pancawara, saptawara: &Saptawara) -> Self {
        ...
        match sum {
            0 => Dasawara::Pandita, 1 => Dasawara::Pati,    2 => Dasawara::Suka,
            3 => Dasawara::Duka,    4 => Dasawara::Sri,      5 => Dasawara::Manuh,
            6 => Dasawara::Manusa,  7 => Dasawara::Raja,     8 => Dasawara::Dewa,
            _ => Dasawara::Raksasa,
        }
    }
    pub fn name(&self) -> &'static str {
        match self {
            Dasawara::Pandita => "Pandita", Dasawara::Pati    => "Pati",
            Dasawara::Suka    => "Suka",    Dasawara::Duka    => "Duka",
            Dasawara::Sri     => "Sri",     Dasawara::Manuh   => "Manuh",
            Dasawara::Manusa  => "Manusa",  Dasawara::Raja    => "Raja",
            Dasawara::Dewa    => "Dewa",    Dasawara::Raksasa => "Raksasa",
        }
    }
    pub fn urip(&self) -> u8 {
        match self {
            Dasawara::Pandita => 7, Dasawara::Pati    => 1, Dasawara::Suka    => 4,
            Dasawara::Duka    => 8, Dasawara::Sri     => 6, Dasawara::Manuh   => 3,
            Dasawara::Manusa  => 5, Dasawara::Raja    => 8, Dasawara::Dewa    => 8,
            Dasawara::Raksasa => 9,
        }
    }
}
