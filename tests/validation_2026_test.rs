...
fn pawukon_may_through_december() {
    ...
    // Aug 17: Soma Umanis Medangkungan — HUT RI ke-81
    ...
    // Dec 5: Saniscara Umanis Tolu
    let d = BalineseDate::from_ymd(2026, 12, 5).unwrap();
    assert_eq!(d.wuku.name(), "Tolu");
    assert_eq!(d.saptawara.name(), "Saniscara");
    assert_eq!(d.pancawara.name(), "Umanis");
...
fn saka_year_2026() {
    ...
    let d = BalineseDate::from_ymd(2026, 3, 18).unwrap();
    assert_eq!(d.saka_year, 1947);

    // Nyepi day (Mar 19, 2026) and after: Saka 1948
    // Note: Nyepi itself is the first day of new Saka year
    let d = BalineseDate::from_ymd(2026, 3, 19).unwrap();
    assert_eq!(d.saka_year, 1948);

    let d = BalineseDate::from_ymd(2026, 3, 20).unwrap();
    assert_eq!(d.saka_year, 1948);
...
fn rahinan_saraswati_pagerwesi_2026() {
    ...
    // From kalenderbali.org: Oct 31, 2026
    let d = BalineseDate::from_ymd(2026, 10, 31).unwrap();
...
fn rahinan_tumpek_2026() {
    ...
    // Tumpek Wariga (Uduh): May 23, Dec 19 (per kalenderbali.org)
    let d = BalineseDate::from_ymd(2026, 12, 19).unwrap();
...
fn pawukon_cycle_completes_in_210_days() {
    ...
    // Second cycle: Jul 30 + 210 = Feb 25, 2027 (420 days from Jan 1)
    let d3 = BalineseDate::from_ymd(2027, 2, 25).unwrap();
